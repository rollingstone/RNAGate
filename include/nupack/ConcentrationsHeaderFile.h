...
#ifndef CONCENTRATIONSHEADER_H_
#define CONCENTRATIONSHEADER_H_
...
// Function prototypes
/* ******************************* IN CALCCONC.C ********************************** */
void getInitialGuess(double *x01, double *lambda1, double *G1, int **AT, int **A1,
		     int numSS, int numTotal, double PerturbScale, long *idum1);
...
void PerturbLambda(double *lambda, double PerturbScale, double *G, int **AT, 
		   int numSS, int numTotal, long *idum);
...
int CalcConc(double *x, int **A, double *G, double *x0, int numSS, int numTotal, 
	     ...
	     int WriteLogFile, char *logFile, double MolesWaterPerLiter, long seed);
/* ******************************************************************************** */
...
void ReadCommandLine(int nargs, char **args, char *cxFile, char *conFile, 
		     ...
		     int *DoBPfracs, long *seed, double *cutoff);
...
long GetRandSeed(long s); // Gets the seed for the random number generator
float ran1(long *idum); // Random number generator in this code
