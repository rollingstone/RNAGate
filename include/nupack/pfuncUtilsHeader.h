...
#ifndef FUNCTIONS_NUPACK_H
#define FUNCTIONS_NUPACK_H
...
enum { DNA_1, RNA_1, RNA37_1, USE_SPECIFIED_PARAMETERS_FILE, COUNT_1};
enum { FALSE, TRUE}; 

//oneDnaStruct and dnaStructures are used for enumerating sequences
typedef struct {
	int *theStruct; //describes what is paired to what
	DBL_TYPE error; //accumalated error (from the mfe) for a structure
...
/*
     ...
     (T in degrees C).
 */
double WaterDensity_NUPACK(double T1);
...
DBL_TYPE pfuncFull( char inputSeq[], int complexity, int naType, 
...
DBL_TYPE mfeFull( char *inputSeq, int *thepairs, int complexity, 
		int naType,
...
DBL_TYPE mfeFullWithSym( char inputSeq[], dnaStructures *mfeStructures, 
		int complexity, int naType,
		int dangles, DBL_TYPE temperature,
		int symmetry, int onlyOne);

// Same as above, but with progress reporting
DBL_TYPE mfeFullWithSym_Progress( char inputSeq[], dnaStructures *mfeStructures, int complexity, 
		int naType, int dangles, DBL_TYPE temperature, int symmetry,
...
// Same as above, but with progress reporting
DBL_TYPE mfeFullWithSym_SubOpt_Progress( char inputSeq[], dnaStructures *mfeStructures, 
		int complexity, int naType,
...
DBL_TYPE naEnergyFull( char *prefix, char *seq, int naType, int dangles, DBL_TYPE temp);

//Same as naEnergyFull(), but allows possible symmetry (see above)
DBL_TYPE naEnergyFullWithSym( char prefix[], char inputSeq[], int naType, 
		int dangles, DBL_TYPE temperature, int possibleSym);

//computes energy of secondary structure in file, using same assumptions as pfunc
DBL_TYPE naEnergy( char *prefix, char *seq);
...
void MakeFold( fold_nupack *thefold, int seqlength, char *seq, char *parens, int *thepairs);
/* ******************************************************************************** */
...
/* processMultiSeqence() copies input sequence (containing '+' characters) to seq, but without the
   '+' characters.  The location of the strand breaks, '+', are stored in the nicks array */
void processMultiSequence( char *inputSeq, int seqlength, int nStrands,
		char *seq, int nicks[]);
...
void nonZeroInit( DBL_TYPE Q[], char seq[], int seqlength);
...
int EtaNIndex( float i, float j, int seqlength);
...
void LoadFold( fold_nupack *thefold, char filename[]);
...
void addDnaStructures( dnaStructures*, const dnaStructures*, const DBL_TYPE newErr,
...
DBL_TYPE DangleEnergy( int i, int j, char seq[], int seqlength);

//Calculates exp(-(dangle energy)/RT) and 
//exp( -(interior loop energy)/RT), respectively 
DBL_TYPE ExplDangle( int i, int j, char seq[], int seqlength);
DBL_TYPE ExplInternal( int i, int j, int h, int m, char seq[]);
...
DBL_TYPE NickedEmptyQ( int i, int j, const int nicks[], char *seq,
		int seqlength, int **etaN);
DBL_TYPE NickedEmptyF( int i, int j, const int nicks[], char *seq,
		int seqlength, int **etaN);
...
/* ******************************************************************************** */
//Hairpin energy (exp)
DBL_TYPE ExplHairpin( int i, int j, char seq[], int seqlength, int **etaN);

//Calculates the contribution to the partition function of multiloops (non-nicked) 
DBL_TYPE SumExpMultiloops( int i, int j, char *seq, 
		DBL_TYPE *Qms, DBL_TYPE *Qm, int seqlength,
		int **etaN);
//Calculates the contribution of exterior loops
DBL_TYPE SumExpExteriorLoop( int i,int j, char *seq, int seqlength, 
		DBL_TYPE *Q,
		int *nicks, int **etaN);
...
DBL_TYPE SumExpInterior_Multi( int i, int j, char seq[], int seqlength, 
...
DBL_TYPE SumExpInextensibleIL( int i, int j, char seq[], int seqlength, 
...
DBL_TYPE SumExpQb_Pk( int i, int j, char *seq, int seqlength, 
...
DBL_TYPE SumExpQp_N5( int i, int j, char *seq, int seqlength, 
...
void MakeQg_N5( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg,
		DBL_TYPE *Qm, DBL_TYPE *Qgls, DBL_TYPE *Qgrs, DBL_TYPE *QgIx,
		DBL_TYPE *QgIx_2, short *possiblePairs);
void MakeQgls( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg, 
		DBL_TYPE *Qm,  DBL_TYPE *Qgls);
void MakeQgrs( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg, 
...
void calculatePairsN3( DBL_TYPE *Q, DBL_TYPE *Qb, DBL_TYPE *Qm, DBL_TYPE *Qms,
		...
		char *seq, int *nicks, int** etaN);

void MakeP_Pm_N3( int i, int j, char *seq, int seqlength, 
		DBL_TYPE *Q, DBL_TYPE *Qs,
		DBL_TYPE *Qms, DBL_TYPE *Qm,
		DBL_TYPE *P,  DBL_TYPE *Ps,
		DBL_TYPE *Pms, DBL_TYPE *Pm,
		int **etaN);
void MakePs_Pms( int i, int j, char *seq, int seqlength, 
		DBL_TYPE *Qs, DBL_TYPE *Qms, DBL_TYPE *Qb,
		DBL_TYPE *Ps, DBL_TYPE *Pms, DBL_TYPE *Pb,
		int *nicks, int **etaN);

//Consider Exterior loops
void prExterior_N3( int i,int j, char *seq, int seqlength, 
		DBL_TYPE *Q, DBL_TYPE *Qb,
		DBL_TYPE *P, DBL_TYPE *Pb,
		int *nicks, int **etaN);

//Consider multiloops
void prMultiBp_N3( int i, int j, char *seq, int seqlength,
...
void smallInteriorLoop( int pf_ij, char *seq, int seqlength, int i, int j, 
...
void MakePgrs( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg, 
		DBL_TYPE *Qm, DBL_TYPE *Qgrs, DBL_TYPE *Pg, DBL_TYPE *Pm,
		DBL_TYPE *Pgrs);

void MakePgls( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg, 
		DBL_TYPE *Qm,  DBL_TYPE *Qgls, DBL_TYPE *Pg, DBL_TYPE *Pm,
		DBL_TYPE *Pgls);

void MakePg_N5( int i, int j, char *seq, int seqlength, DBL_TYPE *Qg,
...
void recalculateQx( int i, int j, int size, int fbix, char *seq, 
		int seqlength, DBL_TYPE *Qx, DBL_TYPE *Qb,
		int *nicks, int **etaN,
		int side);
void recalculateQgIx( int i, int j, int d, int e, int size, int qgix, char *seq, 
		int seqlength, DBL_TYPE *QgIx, DBL_TYPE *Qb,
		int side);


//Calculate interior loop probabilities the slow, N^4 way.
void prInteriorLoopsN4MS(int i, int j, char *seq, int seqlength,
...
DBL_TYPE MinInextensibleIL( int i, int j, char seq[], int seqlength, 
...
DBL_TYPE MinFb_Pk( int i, int j, char *seq, int seqlength, DBL_TYPE *Fp, 
		DBL_TYPE *Fm);

//Fill out minimum values for all Fg matrices with outer pair i,j
void MakeFg_N5( int i, int j, char *seq, int seqlength, DBL_TYPE *Fg,
...
void MakeFgls( int i, int j, char *seq, int seqlength, DBL_TYPE *Fg, 
		DBL_TYPE *Fm,  DBL_TYPE *Fgls);
void MakeFgrs( int i, int j, char *seq, int seqlength, DBL_TYPE *Fg, 
...
DBL_TYPE MinFp_N5( int i, int j, char *seq, int seqlength, 
...
void bktrF_Fm_N3( int i, int j, char *seq, int seqlength, 
		...
		const DBL_TYPE mfeEpsilon,
		const int onlyOne);
void bktrFs_Fms( int i, int j, char *seq, int seqlength, 
		const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
...
void bktrFb_N3( int i, int j, char *seq, int seqlength, const DBL_TYPE *F, const DBL_TYPE *Fb, 
		const DBL_TYPE *Fm, const DBL_TYPE *Fs, const DBL_TYPE *Fms,
		const int *nicks, int **etaN, dnaStructures *dnaStr,
		const int *maxILoopSize,
		const DBL_TYPE mfeEpsilon,
		const int onlyOne);
int bktrMinMultiloops( int i, int j, char *seq, int seqlength,
		const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
		...
		const DBL_TYPE mfeEpsilon,
		const int onlyOne);
int bktrMinExteriorLoop( int i, int j, char *seq, int seqlength,
		const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
		const DBL_TYPE *Fs, const DBL_TYPE *Fms, const int *nicks,
		int **etaN, dnaStructures *dnaStr,
		const int *maxILoopSize,
		const DBL_TYPE mfeEpsilon,
		const int onlyOne);
int bktrMinInteriorLoop( int i, int j, char *seq, int seqlength,
		const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
		const DBL_TYPE *Fs, const DBL_TYPE *Fms, const int *nicks,
		int **etaN, dnaStructures *dnaStr,
		const int *maxILoopSize,
...
//complexity = 5
		void bktrF_Fm_FzN5( int i, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
		...
		void bktrFbN5( int i, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
		...
		void bktrFgN5( int i, int d, int e, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr,  dnaStructures *dnaStr,
		...
		void bktrFpN5( int i, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr, dnaStructures *dnaStr, const int *nicks,
		...
		void bktrFgls( int i, int d, int e, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr, dnaStructures *dnaStr,
		...
		void bktrFgrs( int i, int d, int e, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr, dnaStructures *dnaStr,
		...
		void bktrFgl(  int i, int e, int f, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr, dnaStructures *dnaStr,
		...
		void bktrFgr(  int i, int d, int e, int j, char *seq, int seqlength,
				const DBL_TYPE *F, const DBL_TYPE *Fb, const DBL_TYPE *Fm,
				const DBL_TYPE *Fp, const DBL_TYPE *Fz, const DBL_TYPE *Fg,
				const DBL_TYPE *Fgls, const DBL_TYPE *Fgrs, const DBL_TYPE *Fgl,
				const DBL_TYPE *Fgr, dnaStructures *dnaStr,
		...
		DBL_TYPE GetEnergy( fold_nupack *thefold);

		//Calculate the energy of an exterior loop substructure
		DBL_TYPE EnergyF( int start, int stop, fold_nupack *thefold);
		//Energy of Paired substructure
		DBL_TYPE EnergyFb( int start, int stop, fold_nupack *thefold);
		//Energy of pseudoknotted substructure
		DBL_TYPE EnergyPk( int i, int j, fold_nupack *thefold);
		//Energy of gap spanning region
		DBL_TYPE EnergyFg( int i, int d, int e, int j, fold_nupack *thefold);
		//Energy of pseudoknot interior
		DBL_TYPE EnergyFz( int start, int stop, fold_nupack *thefold);
		/* ******************************************************************************** */
